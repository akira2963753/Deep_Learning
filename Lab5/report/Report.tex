...
As a bonus, Dueling networks and NoisyNet were each stacked on the Task~3 baseline; neither
beat the baseline at the same checkpoints. For Dueling, a follow-up with PER disabled
tracked the PER-on run almost exactly, so the gap is unlikely to be a PER--architecture
interaction.
...
All source files and scripts are located in the \texttt{LAB5\_M11407439\_Code/} directory.
...
Run the provided shell scripts, or use the commands below directly.

\vspace{0.2cm}

\textbf{Task 1 : CartPole-v1:}
\begin{pycode}
python dqn_task1.py \
...
\textbf{Task 3 : Pong (DDQN + PER + Multi-step):}
\begin{pycode}
python dqn_task3.py \
  --env ALE/Pong-v5 \
  --num-envs 4 \
...
Evaluation scripts run 20 episodes under the greedy policy and save recordings to
\texttt{./eval\_videos\_task\{1,2,3\}/}.
Model weights (\texttt{.pt} files) should be placed one level above the code directory
(i.e.\ alongside \texttt{LAB5\_M11407439\_Code/}), as named below.

\textbf{Task 1 : CartPole-v1:}
\begin{pycode}
python test_model_task1.py \
  --model-path ../LAB5_M11407439_task1.pt \
  --episodes 20 \
  --seed 0 \
...
python test_model_task2.py \
  --model-path ../LAB5_M11407439_task2.pt \
  --episodes 20 \
  --seed 0 \
  --output-dir ./eval_videos_task2
\end{pycode}

\textbf{Task 3 : Pong (DDQN + PER + Multi-step)} (example: 1\,M-step checkpoint):
\begin{pycode}
python test_model_task3.py \
  --model_path ../LAB5_M11407439_task3_1000000.pt \
  --episodes 20 \
  --seed 0
\end{pycode}

Alternatively, the shell scripts \texttt{train\_task\{1,2,3\}.sh} and
\texttt{eval\_task\{1,2,3\}.sh} wrap the above commands and can be run directly
with \texttt{bash train\_task1.sh}, etc.
...
Table 2.3 summarises all training configurations and strategies I attempted, among 
which Run1 achieved the best overall performance; its detailed evaluation results are presented below.
...
Table~\ref{tab:sample-efficiency} summarises the 20-seed average evaluation reward at each
checkpoint for both configurations.

\begin{table}[h]
\centering
\caption{20-seed average evaluation reward at each checkpoint:
         \textit{task3-full}vs.\textit{task3-none}}
...
task3-none (vanilla DQN)             & $-$15.25 & 16.75 & 16.75 & 16.25 & 14.10 \\
\bottomrule
\end{tabular}}
\end{table}

\paragraph{Overall convergence speed.}
\textit{task3-full} first crosses score~19 at the 1\,M-step checkpoint (average reward 19.35)
and continues climbing steadily to 20.55 by 2.5\,M.
\textit{task3-none}, by contrast, never reaches a comparable level within 2.5\,M steps,
peaking at only 14.10, making the enhanced stack roughly $3\times$ more sample-efficient
at the critical threshold.

\paragraph{Early-phase analysis (0--600\,k steps).}
The gap is most pronounced in the early phase: at 600\,k steps,
\textit{task3-full} already achieves a reward of 17.35, while \textit{task3-none} is still near zero.
Two enhancements account for this early advantage.
\textbf{Prioritised Experience Replay (PER)} re-samples transitions with high TD error more
frequently, so the agent extracts more useful gradient signal from the same number of
environment interactions; even a partially-filled replay buffer yields high-quality batches.
\textbf{Multi-step return} ($n{=}3$) replaces the single-step Bellman target with a
3-step accumulated reward, propagating real environmental feedback further into the
Q-value estimates and reducing the bootstrapping lag that slows vanilla DQN in the first
few hundred thousand steps.
Together these two techniques let \textit{task3-full} learn a winning policy before
\textit{task3-none} has meaningfully moved off random-play reward.

\paragraph{Stability analysis.}
Beyond speed, \textit{task3-none} also fails to converge: its best average reward across
the entire 2.5\,M-step run is only 14.10, well below the target score of 19.
Its learning curve collapses twice (visible near 1.25\,M and 2.25\,M steps in
Figure~\ref{fig:sample-efficiency}), dropping back toward zero each time and never
recovering to a stable winning policy.
I attribute these collapses to overestimation bias in vanilla DQN---
the online network's max operator inflates Q-value targets, leading to runaway estimates
that periodically destabilise the policy.
\textbf{Double DQN (DDQN)} decouples action selection from value estimation, which
suppresses this bias and prevents the catastrophic regressions seen in \textit{task3-none}.
Its primary contribution is therefore not to accelerate the initial rise but to
maintain the policy once the other two techniques have gotten it off the ground,
producing the smooth, monotonically increasing curve seen for \textit{task3-full}.
...
Stacking Dueling on \textit{task3-full} lags the baseline at every milestone in
Table~\ref{tab:ablation-milestones}: 11.40 vs.\ 17.35 at 600\,k, and 18.35 vs.\ 20.55 by
2.5\,M. Between 1.5\,M and 2\,M the score even dips (18.30 to 17.90), so the run never
really settles.

We first guessed that PER might be the problem. With
$Q(s,a) = V(s) + A(s,a)$, TD errors no longer come from a single head, and we worried that
PER's priorities would track the wrong thing. To test that, we left Dueling, DDQN,
multi-step, and $\text{lr} = 2.5\times10^{-4}$ alone, and compared \textit{task3-full + Dueling}
(with PER) to a run with PER switched off (W\&B: \textit{task3-noPER + Dueling}).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{W&B Chart 2026_5_2 下午6_49_14}
    \caption{Eval reward vs.\ env steps: Dueling with PER (grey) vs.\ Dueling without PER
             (purple), same $\text{lr}=2.5\times10^{-4}$.}
    \label{fig:dueling-per-ablation}
\end{figure}

The two lines in Figure~\ref{fig:dueling-per-ablation} sit on top of each other. Dropping PER
does not fix the gap in Table~\ref{tab:ablation-milestones}, so we drop the story that PER
and Dueling ``fight'' each other. What seems more likely is that the CNN plus DDQN and
multi-step already carries almost everything Pong needs, and Dueling mostly adds parameters
without a payoff on this task. Eval is one Pong episode at a time, so the curves are noisy
anyway. We did not try other losses or learning-rate grids here.
...
\begin{figure}[h]
    ...
    \caption{Same Dueling stack, two learning rates ($2.5\times10^{-4}$ vs.\ $1.8\times10^{-4}$).}
    \label{fig:bonus-curves-dueling}
\end{figure}

Figure~\ref{fig:bonus-curves-dueling} changes the learning rate only. The higher one learns
faster early on but wobbles more; the lower one is calmer later but slower out of the gate.
End-game reward lands in the same band, so tuning $\text{lr}$ alone does not close the
distance to plain \textit{task3-full} either.
...
On Pong, the extra pieces do not pay off uniformly. A follow-up run showed that Dueling
without PER tracks Dueling with PER almost exactly, so the earlier guess about PER was
wrong; the miss is mostly that the baseline already fits this game well. C51 still trains
slower than scalar DQN here. That matches what Rainbow reports: tricks help some games and
do little on others. NoisyNet stays closest to \textit{task3-full} among the three, but even
it ends below the baseline at 2.5\,M steps.
