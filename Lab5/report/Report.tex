...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{W&B Chart 2026_4_30 下午6_56_59.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\textbf{Dueling DQN} is the weaker of the two additions.
It scores only 11.40 at 600\,k steps --- more than 5 points behind \textit{task3-full} ---
and peaks at 18.35 by 2.5\,M, never catching up.
The non-monotonic trajectory (18.30 $\to$ 17.90 $\to$ 18.35) also hints at instability.
The likely cause is an interaction between the dueling architecture and PER.
Dueling splits $Q(s,a) = V(s) + A(s,a)$, so TD errors now reflect a mixture of errors in
the value stream and the advantage stream.
PER's priority mechanism was designed for a single scalar error; with two entangled sources
of noise, the assigned priorities become less reliable, the replay distribution drifts, and
early training is slower.
Pong also has relatively few ``dead'' states where $V(s)$ dominates and advantages wash
out --- the action choice is almost always consequential --- so the architecture provides
little structural benefit to offset the added complexity.


\textbf{NoisyNet} performs closer to the baseline but still lags throughout.
It reaches 19.00 at 1.5\,M steps but plateaus around 19.20 while \textit{task3-full}
continues climbing to 20.55.
NoisyNet replaces the $\varepsilon$-greedy schedule with learnable per-weight noise,
which works well when the exploration schedule is the bottleneck.
Here, however, the $\varepsilon$ decay was already well-tuned for Pong, so NoisyNet
buys little extra exploration benefit.
More importantly, the stochastic forward passes add variance to the Q-value estimates
used by PER to compute priorities, introducing noise into the very signal PER depends on
to rank transitions.
This likely explains the slower convergence and lower asymptotic score relative to the
clean DDQN\,+\,PER\,+\,Multi-step combination.
