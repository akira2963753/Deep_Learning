...
Task~2 extends DQN to the Atari game environment (\texttt{ALE/Pong-v5}).
Unlike CartPole, the agent receives raw pixel frames as input rather than a low-dimensional
state vector, requiring a convolutional neural network to extract spatial features.
The core DQN algorithm (Experience Replay, Target Network, $\varepsilon$-greedy) remains
identical to Task~1; the key differences lie in the network architecture and the
...
Task~3 builds on the Task~2 Atari agent by incorporating three complementary
improvements from the Rainbow DQN family:
...
\begin{itemize}[leftmargin=1.8em]
  \item \textbf{Double DQN (DDQN)}: Decouples action \emph{selection} (online network)
        ...
        eliminating the systematic overestimation bias present in vanilla DQN.
  ...
  \item \textbf{Prioritized Experience Replay (PER)}: Replaces uniform random sampling
        ...
        Transitions the network has not yet learned from receive higher sampling probability,
        improving sample efficiency.
        Importance-Sampling (IS) weights correct the resulting distribution bias.

  \item \textbf{Multi-step Return ($n$-step)}: Instead of bootstrapping from the very next
        state, the agent accumulates $n = 3$ discounted rewards before appending the
        bootstrap term $\gamma^n Q_{\text{target}}(s_{t+n})$.
        This accelerates reward propagation and reduces reliance on an
        imprecise Q-value estimate.
\end{itemize}
...
In uniform experience replay, sampling a mini-batch from a \texttt{deque} of size $N$
costs $O(N)$ time.
Prioritized Experience Replay (PER, Schaul et al.\ 2016) assigns each transition a
\textbf{priority} $p_i = |\delta_i|^\alpha + \varepsilon$ proportional to its TD error,
and samples proportionally to these priorities.
A naïve linear scan to find a weighted sample would still be $O(N)$.
To achieve $O(\log N)$ for both insertion and sampling,
PER uses a \textbf{SumTree} — a binary segment tree whose internal nodes store the
sum of all their descendants' priorities.
...
the max over noisy estimates is systematically biased upward —
a phenomenon known as \textbf{overestimation bias}.
This can cause the agent to learn overly optimistic Q-values,
...
Figure~\ref{fig:sample-efficiency} compares the learning curves of the two agents.
Several observations stand out:

\begin{itemize}[leftmargin=1.8em]
  \item \textbf{Faster convergence.}
        The enhanced agent (\textit{task3-full}) begins to improve from the very first
        evaluation, reaching an average reward of approximately 19 by \textbf{600\,k} steps.
        In contrast, the vanilla baseline (\textit{task3-none}) remains near $-20$ until
        roughly 400\,k steps and does not consistently exceed 19 until beyond 2\,M steps —
        a roughly \textbf{3$\times$ reduction in the number of environment steps required}.

  \item \textbf{Greater stability.}
        After converging, \textit{task3-full} fluctuates narrowly between 18 and 21,
        maintaining a stable plateau throughout the rest of training.
        The baseline exhibits far higher variance, including two catastrophic drops
        (to near $-1$ at $\sim\!1.25$\,M steps and near $+3$ at $\sim\!2.25$\,M steps),
        indicating policy instability typical of unmitigated overestimation bias and
        poorly distributed replay samples.

  \item \textbf{Contribution of each enhancement.}
        The accelerated early learning is primarily attributable to \textbf{PER}
        (high-error transitions are replayed more often, producing denser learning signal)
        and \textbf{Multi-step return} (faster reward propagation reduces the number of
        updates needed to back-propagate a sparse end-of-game reward).
        \textbf{Double DQN} suppresses overestimation, which explains the improved
        long-run stability of \textit{task3-full} compared to \textit{task3-none}.
\end{itemize}
